...
| CWT (cmor-1.0-1.0, 60 scales) | Tensor (19, 60, 1000) — 19-channel scalogram matrices | 20 sec splits, %70 train, %10 val, %20 test | **One-vs-Rest (OvR) + Val-Threshold Opt.**: 3× YOLO26m-cls binary models (same backbone & init); per-model class weights from distribution; val-set grid search (step=0.05) selects per-class thresholds only by using validation probabilities; 3-class decision by argmax(P − thresholds) at both segment and subject level. Binary test results — model_ad: Acc=%75.41, F1=%72.87, Prec=%64.42, Rec=%83.86, Train~%90.83; model_ftd: Acc=%73.76, F1=%43.79, Prec=%48.05, Rec=%40.22, Train~%96.18; model_hc: Acc=%83.01, F1=%77.43, Prec=%72.76, Rec=%82.75, Train~%95.63 | Dropout= 0.20, LR schedule= cosine decay (T_max=200), No early stopping, Epochs= 200, Batch= 8, Learning rate= 0.0001, Weight decay= 0.0005, Optimizer= AdamW, Loss= Weighted Binary Cross Entropy (label smoothing=0.01), Grad clip= 2.0, Val-optimized thresholds: AD=0.20–0.40, FTD=0.05, HC=0.15–0.35 | **Test= %69.20** (seg) / **%88.89** (subj), Train= ~%91-96 | **Test= %69.55** (seg) / **%88.77** (subj), Train= ~%91-96 | **Test= %70.31** (seg) / **%89.58** (subj), Train= ~%91-96 | **Test= %69.20** (seg) / **%88.89** (subj), Train= ~%91-96 | ... |
...
Ammar Omar - ahatim030@gmail.com  <br> 
